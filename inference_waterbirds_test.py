...
# Get the test set
id_test_data = dataset.get_subset('test',
                    transform=transforms.Compose([transforms.Resize((448,448)),
                                                  transforms.ToTensor()]))

# Prepare the standard data loader
id_test_loader = get_eval_loader('standard', id_test_data, batch_size=16)
...
checkpoint = torch.load(os.path.join('model/waterbirds_seed:0_epoch:best_model.pth'), map_location=torch.device('cuda'))
...
with torch.no_grad():
    for batch_num, batch in tqdm(enumerate(id_test_loader), total=len(id_test_loader), position=0, leave=True):
        inputs = batch[0].cuda()
...
saveResults(os.path.join('inference_results', 'waterbirds_test_resnet.pkl'), results)